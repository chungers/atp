...
TEST(DbTest, DbReadWriteMarketDataTest)
{
  ...
  key << "AAPL.STK" << ":" << atp::common::as_micros(t);

  std::ostringstream key2;
  key2 << "AAPL.STK" << ":" << atp::common::as_micros(t) + 1;
  ...
  ///
  {
    ...
    qbs.set_utc_first_micros(atp::common::as_micros(t));

    boost::posix_time::time_duration td(0, 0, 0, 1);
    qbs.set_utc_last_micros(atp::common::as_micros(t + td));
    qbs.set_index("ASK");
