...
fh_opt_contract_detail <- function(ibg, symbol) {
  ...
  # contract details is a list, get the first one.
  message('Getting option chain for ', symbol, q)

  l <- reqContractDetails(ibg, q)

  # combine into a map where keys are the symbols and values are the contractDetails
  opt <- list()
  # index by opt$strike$right$expiry
  lapply(l, function(x) {
    k<-as.character(x$contract$strike)
    r<-as.character(x$contract$right)
    e<-as.character(x$contract$expiry)
    ifelse(is.null(opt[[k]]),
           opt[[k]] <<- list(),
           ifelse(is.null(opt[[k]][[r]]),
                  opt[[k]][[r]] <<- list(),
                  ifelse(is.null(opt[[k]][[r]][[e]]),
                         opt[[k]][[r]][[e]] <<-x,
                         opt[[k]][[r]][[e]] <<-x)))
  })
  ret <- list()
...
fh_load_opt_contract_details <- function(ibg, symbols) {
  list <- foreach(s = symbols, .combine="c") %do% fh_opt_contract_detail(ibg, s)
  return(list)
}
